...
def valid_outline_target_objects(scene, view_layer=None):
    objects = []
    skipped = 0
    view_objects = set(getattr(view_layer, "objects", [])) if view_layer is not None else None
    for item in getattr(scene, "pixel_render_outline_targets", []):
        ...
        if obj is None or getattr(obj, "type", None) not in SUPPORTED_TYPES or getattr(obj, "hide_render", False):
            skipped += 1; continue
        if view_objects is not None and obj.name not in view_objects and obj not in view_objects:
            skipped += 1; continue
        objects.append(obj)
...
def render_outline_target_mask(scene, view_layer, width, height):
    ...
    try:
        with temporary_object_index_state(scene, view_layer, objects):
